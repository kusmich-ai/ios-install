 ...
 * 
 * Schedule-aware: only counts practices that were SCHEDULED for each day.
 * For example, flow_block is weekday-only, so weekends expect fewer practices.
 * 
 * @param {Array} practiceLogs - Array of practice log entries
 * @param {number} stage - Current stage
 * @param {number} days - Number of days in the rolling window (default 14)
 * @param {number|null} daysInStage - Actual days user has been in stage (caps the window)
 * @returns {number} Adherence percentage (0-100)
 */
export function calculateAdherence(practiceLogs, stage, days = 14, daysInStage = null) {
  // Use the smaller of: rolling window OR actual days in stage
  // This prevents new users from being penalized for days that haven't happened yet
  const effectiveDays = daysInStage !== null 
    ? Math.min(Math.max(daysInStage, 1), days) 
    : days;
  ...
  for (let i = 0; i < effectiveDays; i++) {
    ...
    // Get only the practices SCHEDULED for this specific day of the week
    const scheduledPractices = getScheduledPracticeIdsForDate(stage, checkDate);
    totalExpected += scheduledPractices.length;

    // Find completed practices for this date
    const dayLogs = practiceLogs.filter(
      log => log.practice_date === dateStr && log.completed
    );
 ...
 * 
 * Schedule-aware: a day only requires the practices that were SCHEDULED for it.
 * Weekends won't break streaks for weekday-only practices like flow_block.
 * 
 * @param {Array} practiceLogs - Array of practice log entries sorted by date desc
 * @param {number} stage - Current stage
 * @returns {number} Number of consecutive days with all scheduled practices completed
 */
export function calculateConsecutiveDays(practiceLogs, stage) {
  ...
  // Walk backwards from today, checking up to 60 days
  for (let i = 0; i < 60; i++) {
    const checkDate = new Date(today);
    checkDate.setDate(checkDate.getDate() - i);
    const dateStr = checkDate.toISOString().split('T')[0];
    ...
    // If no practices are scheduled for this day, skip it (don't break streak)
    if (scheduledPractices.length === 0) {
      continue;
    }

    const completedPractices = logsByDate[dateStr] || new Set();

    // Check if ALL scheduled practices were completed
    const allScheduledCompleted = scheduledPractices.every(
      practiceId => completedPractices.has(practiceId)
    );

    if (allScheduledCompleted) {
      consecutiveDays++;
    } else if (dateStr === today.toISOString().split('T')[0]) {
      // Today is incomplete but we might have consecutive days before — skip today
      continue;
    } else {
      break; // Streak broken
    }

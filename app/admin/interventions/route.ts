...
// ============================================
// HELPER: Send email via your email provider
// Replace this with your actual email sending logic
// (Resend, SendGrid, Postmark, or Supabase Edge Function)
// ============================================
async function sendEmail(to: string, subject: string, html: string): Promise<boolean> {
  try {
    // Option 1: If you have a /api/email/send route already
    ...
    // Option 2: If using Resend
    // const resend = new Resend(process.env.RESEND_API_KEY);
    // await resend.emails.send({ from: 'nicholas@unbecoming.app', to, subject, html });
    // return true;

    // Option 3: If using a Supabase Edge Function for email
    ...
    return true; // Return true so it logs to notification_log
  } catch (error) {
...
export async function POST(req: Request) {
  try {
    ...
    if (action === 'sendIntervention') {
      ...
      const firstName = targetUser?.first_name || 'there';

...
Output JSON in this exact structure (no markdown, no backticks, just raw JSON):
{
  "milestones": [
    {
      "number": 1,
      ...
      "whats_broken": "Your 'Fight-or-flight default' keeps you in survival mode. You can't access clarity because your body is running threat detection 24/7.",
      "what_changes": [
  ...
  "destination": {
    ...
    "liberation_statement": "When you complete this roadmap, this pattern will no longer run your life. You'll still achieve - but from wholeness, not wound."
  }
}

Create 3-4 milestones maximum. Output ONLY valid JSON - no explanation, no markdown.`;
...
export async function GET(request: NextRequest) {
  return NextResponse.json({ 
    status: 'ok', 
    message: 'Regenerate endpoint is working. Use POST to generate roadmap.' 
  });
}

// ============================================
// POST handler - Main regeneration logic
// ============================================
export async function POST(request: NextRequest) {
  console.log('[Regenerate] Starting roadmap regeneration...');
  ...
  try {
    ...
    if (authError || !user) {
      console.log('[Regenerate] Auth error:', authError);
      return NextResponse.json(
        { error: 'Unauthorized' },
        { status: 401 }
      );
    ...
    console.log('[Regenerate] User authenticated:', user.id);
    ...
    if (fetchError || !profile) {
      console.log('[Regenerate] Profile fetch error:', fetchError);
      return NextResponse.json(
    ...
    console.log('[Regenerate] Profile found, collecting patterns...');
    ...
    console.log('[Regenerate] Found', allPatterns.length, 'patterns');
...
`;
    ...
    console.log('[Regenerate] Claude response received');
    ...
    console.log('[Regenerate] Response text length:', responseText.length);
    ...
    try {
      ...
      console.log('[Regenerate] JSON parsed successfully');
    } catch (parseError) {
      console.error('[Regenerate] Failed to parse roadmap JSON:', parseError);
      console.error('[Regenerate] Raw response:', responseText.substring(0, 500));
      return NextResponse.json(
        { error: 'Failed to generate roadmap. The AI response was not valid JSON. Please try again.' },
        { status: 500 }
      );
    }
    ...
    if (updateError) {
      console.error('[Regenerate] Failed to update pattern profile:', updateError);
      return NextResponse.json(
